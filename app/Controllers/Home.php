	...
	public function _preset(...$allowed)
	{
		...
		if ($this->session->get($this->log_status) == null) {
			...
			if ($skl->status == 0) {
				...
				$this->session->setFlashdata('error', "Your school is locked by Somanet admin");
	...
	function upload_image($type)
	{
		...
		if ($type == "student_picture") {
			...
			if ($file->move(FCPATH . "assets/images/background", $name)) {
				...
				$sklMdl = new SchoolModel();
				try {
					$sklMdl->save(array("id" => $this->session->get("soma_school_id"), "card_background" => $name));
				} catch (\Exception $e) {
			...
			if ($file->move(FCPATH . "assets/images/background", $name)) {
				...
				$sklMdl = new SchoolModel();
				try {
					$sklMdl->save(array("id" => $this->session->get("soma_school_id"), "sf_card_background" => $name));
				} catch (\Exception $e) {
			...
			if ($file->move(FCPATH . "assets/images/signatures", $name)) {
				...
				$sklMdl = new SchoolModel();
				try {
					$sklMdl->save(array("id" => $this->session->get("soma_school_id"), "headmaster_signature" => $name));
				} catch (\Exception $e) {
			...
			if ($file->move(FCPATH . "assets/images/logo", $name)) {
				...
				$sklMdl = new SchoolModel();
				try {
					$sklMdl->save(array("id" => $this->session->get("soma_school_id"), "logo" => $name));
				} catch (\Exception $e) {
...
//					$pending['remaining_sms'] = $pending['sms_limit'] - $pending['sms_usage'] + $pending['extra_sms'];
					...
					if ($this->_send_sms($pending['phone'], $pending['content'], $result, $pending['remaining_sms'], $pending['acronym'], $school_account)) {
						//increment used sms
...
//		$data['activeStudent'] = count($Mdl->select("id")->groupBy("student_id")->where("status",1)->get()->getResultArray());
	 ...
	 */
	public
	function manipulateStudentSelfRegistration(): Response
	{
