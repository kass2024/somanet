		...
		<div style="margin-top:15px;width: 100%;position: relative" class="<?= $pdf ? 'printable' : 'no-printable'; ?>">
			<div class="col-md-12 col-sm-12 pull-left solid" style="height: 100%;position: relative">
				<div style="background:white;padding: 10px;overflow: auto">
					<table class="tablepage" style="font-size: medium;">
						<tr>
							<td valign="top">
					...
					<table class="tablepage" border="1">
						...
						<tr>
							...
							<td class="colscenter"><?= lang("app.fAssess"); ?> </td>
							<td class="colscenter"><?= lang("app.iAssess"); ?> </td>
					...
					<td colspan=\"10\" style=\"background-color: #7c7c7c\"><b>" . strtoupper($core['category']) . "</b></td>
						...
						<tr>
							...
							$row_total = ($row_exam + $row_cat) / 2;
							$reAssessment = \App\Controllers\Home::reAssessment($core['id'],$student['id'],$term,$year);
							$observationMarks = $reAssessment==null?$row_total:$reAssessment['marks'];
							//						$row_total=($datas['marks'] + $datas['exam_marks']);
							echo "<td>" . number_format($row_cat, 1) . "</td>
								      <td>" . number_format($row_exam, 1) . "</td>
									  <td>" . number_format($row_total, 1) . "</td>
						...
						<tr>
							...
							<td class="colscenter"><?= number_format(($totalCatColumn * 100 / $max_total), 1); ?></td>
							<td class="colscenter"><?= number_format(($totalExamColumn * 100 / $max_total), 1); ?></td>
							<td class="colscenter"><?= number_format((($totalCatColumn + $totalExamColumn) * 100 / ($max_total * 2)), 1); ?>
								%
							</td>
					...
					<!--Table-->
					<footer style="color: darkgrey;position: absolute;bottom: 10px;right: 10px">printed by Somanet</footer>
				</div>
			</div>
		</div>
		<?php
